...
UI_LOCAL="/usr/local/lib/tom_tunnel-ui.sh"
...
prepare_ui() {
    ...
    wget -q -O /usr/local/lib/tom_tunnel-ui.sh "${SERVER_HOST}/module/tom_tunnel-ui.sh" 2>/dev/null || cp "$(dirname "$0")/module/tom_tunnel-ui.sh" /usr/local/lib/tom_tunnel-ui.sh 2>/dev/null || true
    chmod 644 /usr/local/lib/tom_tunnel-ui.sh 2>/dev/null || true
    [ -f /usr/local/lib/tom_tunnel-ui.sh ] && source /usr/local/lib/tom_tunnel-ui.sh
}
...
run_scripts() {
    ...
    for script in "${scripts[@]}"; do
        ...
        log_file="/var/log/tom_tunnel/${script}.log"
...
install_menu() {
    ...
    local_dir="${TOM_TUNNEL_SRC:-}"
    [ -z "$local_dir" ] && local_dir="$(cd "$(dirname "$0")" 2>/dev/null && pwd)"
    for script in dns zivpn expiry domain iptools menu socks ssh status trojan vless vmess netguard port log tgbot uninstall update web fastdns; do
        src=""
        if [ -n "$local_dir" ] && [ -f "${local_dir}/menu/${script}.sh" ]; then
            src="${local_dir}/menu/${script}.sh"
        elif [ -f "./menu/${script}.sh" ]; then
            src="./menu/${script}.sh"
        fi
        if [ -n "$src" ]; then
            install -m 755 "$src" "/usr/local/sbin/$script"
        else
            tmp="/tmp/tom_tunnel-menu-${script}"
            if wget -q --timeout=30 --tries=3 -O "$tmp" "${SERVER_HOST}/menu/${script}.sh"; then
                install -m 755 "$tmp" "/usr/local/sbin/$script"
                rm -f "$tmp"
            else
                printf '%b
' "${K_RED}  ✘ menu/${script}.sh${K_RESET}"
                continue
            fi
        fi
    done
    if [ -n "$local_dir" ] && [ -f "${local_dir}/module/tom_tunnel-ui.sh" ]; then
        install -m 644 "${local_dir}/module/tom_tunnel-ui.sh" /usr/local/lib/tom_tunnel-ui.sh
    elif [ -f "./module/tom_tunnel-ui.sh" ]; then
        install -m 644 "./module/tom_tunnel-ui.sh" /usr/local/lib/tom_tunnel-ui.sh
    fi
...
main() {
    check_root_virt
    check_os
    [ -f /usr/local/lib/tom_tunnel-ui.sh ] && k_brand || true
    setup_host_time
    prepare_ui
    prepare_env
    update_system
    ...
    echo -e "${GR}TOM_TUNNEL is ready. A reboot is recommended, but it is not forced.${NC}"
}
