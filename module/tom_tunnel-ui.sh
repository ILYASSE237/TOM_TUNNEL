#!/bin/bash
# TOM_TUNNEL — Premium terminal UI
...
k_brand(){
  clear
  k_signature
  k_spinner "Initialisation de TOM_TUNNEL" 5
  printf '%b\n' "${K_MAGENTA}╭$(printf '─%.0s' $(seq 1 $((K_WIDTH-2))) )╮${K_RESET}"
  printf '%b\n' "${K_MAGENTA}│${K_RED}  WELCOME TO TOM_TUNNEL • PREMIUM EDITION  ${K_MAGENTA}│${K_RESET}"
  printf '%b\n' "${K_MAGENTA}╰$(printf '─%.0s' $(seq 1 $((K_WIDTH-2))) )╯${K_RESET}"
  printf '%b\n' "${K_GREEN}           TOM_TUNNEL — SECURE SERVER CONTROL${K_RESET}"
  k_sep K_YELLOW
}
