...
[ -f "$UI" ] && k_header "TOM_TUNNEL VPN — OTA UPDATE" || clear
...
MENU=(dns zivpn domain iptools menu socks ssh status trojan vless vmess netguard port log tgbot uninstall update web fastdns)
...
mkdir -p /usr/local/lib/tom_tunnel-core
for f in "${CORE[@]}"; do
  tmp="$TMP_DIR/core-$f"
  k_step "Core/$f"; if fetch "$SERVER_HOST/core/$f.sh" "$tmp"; then chmod +x "$tmp"; install -m 755 "$tmp" "/usr/local/lib/tom_tunnel-core/$f.sh"; k_done; else printf '%b\n' " ${K_RED}✘${K_RESET}"; k_err "Échec de core/$f"; fi
done
...
k_step "UI TOM_TUNNEL"; if fetch "$SERVER_HOST/module/tom_tunnel-ui.sh" "$TMP_DIR/ui"; then install -m 644 "$TMP_DIR/ui" /usr/local/lib/tom_tunnel-ui.sh; k_done; else printf '%b\n' " ${K_RED}✘${K_RESET}"; k_err "UI introuvable"; fi
...
k_step "Launcher TOM_TUNNEL"; if fetch "$SERVER_HOST/tom_tunnel.sh" "$TMP_DIR/tom_tunnel.sh"; then chmod +x "$TMP_DIR/tom_tunnel.sh"; install -m 755 "$TMP_DIR/tom_tunnel.sh" /root/tom_tunnel.sh; k_done; else printf '%b\n' " ${K_RED}✘${K_RESET}"; fi

# Update the web panel only when it is already installed; data/config stay in /etc/tom_tunnel-vpn-web.
if [ -d /opt/tom_tunnel-vpn-web ] && [ -f /opt/tom_tunnel-vpn-web/install.sh ]; then
  printf '%b\n' "${K_MAGENTA}  Web Panel détecté — mise à jour en conservant la configuration.${K_RESET}"
  TOM_TUNNEL_WEB_DIR="/opt/tom_tunnel-vpn-web"; TOM_TUNNEL_REPO_URL="https://github.com/abesskamer237/TOM_TUNNEL.git"; WEB_TMP="$TMP_DIR/repo"
  if git clone --depth 1 "$TOM_TUNNEL_REPO_URL" "$WEB_TMP" >/dev/null 2>&1 && [ -d "$WEB_TMP/tom_tunnel-web" ]; then
    cp -rf "$WEB_TMP/tom_tunnel-web"/. "$TOM_TUNNEL_WEB_DIR/"
    if [ -d "$TOM_TUNNEL_WEB_DIR/frontend" ]; then
      cd "$TOM_TUNNEL_WEB_DIR/frontend" && npm install --quiet >/dev/null 2>&1 && npm run build >/dev/null 2>&1 || true
    fi
    cd "$TOM_TUNNEL_WEB_DIR" && npm install --production=false --quiet >/dev/null 2>&1 && npm run build >/dev/null 2>&1 || true
...
k_sep K_MAGENTA
k_ok "Mise à jour TOM_TUNNEL VPN terminée."
printf '%b\n' "${K_DIM}  Les données /etc/tom_tunnel-vpn-web et /etc/xray ne sont pas supprimées.${K_RESET}"
