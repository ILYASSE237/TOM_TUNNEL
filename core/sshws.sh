...
[ -f "$UI" ] && k_header "TOM_TUNNEL • SSHWS" || clear
...
disable_ipv6() {
    ...
    # Correction du caractère corrompu (cho → echo)
...
configure_nginx() {
    ...
    wget -q --timeout=30 --tries=3 -O /etc/nginx/nginx.conf "${SERVER_HOST}/module/nginx.conf" || {
        echo "[ERROR] Impossible de télécharger nginx.conf"
        return 1
    }
    ...
    if [[ -z "$domain" ]]; then
        echo "[ERROR] Domaine TOM_TUNNEL introuvable dans /etc/xray/domain."
        return 1
    fi
...
setup_web_directories() {
    ...
    wget -q --timeout=30 --tries=3 -O /home/vps/public_html/index.html "${SERVER_HOST}/module/index" || {
        echo "[ERROR] Impossible de télécharger index.html"
        return 1
    }
...
install_badvpn() {
    wget -q --timeout=30 --tries=3 -O /usr/bin/badvpn-udpgw "${SERVER_HOST}/module/newudpgw" || {
        echo "[ERROR] Impossible de télécharger badvpn-udpgw"
        return 1
    }
    chmod +x /usr/bin/badvpn-udpgw
    wget -q --timeout=30 --tries=3 -O /etc/systemd/system/badvpn@.service "${SERVER_HOST}/module/badvpn@.service" || {
        echo "[ERROR] Impossible de télécharger badvpn@.service"
        return 1
    }
    systemctl daemon-reload
...
configure_stunnel() {
    ...
    wget -q --timeout=30 --tries=3 -O "stunnel-${STUNNEL_VERSION}.tar.gz" "$STUNNEL_URL" || {
        echo "[ERROR] Impossible de télécharger stunnel"
        return 1
    }
    ...
    if [[ -f /etc/xray/xray.crt && -f /etc/xray/xray.key ]]; then
        ...
        openssl req -new -x509 -days 1095 -nodes \
            -subj "/C=CM/ST=Centre/L=Yaounde/O=tom_tunnelvpn/OU=stunnel/CN=$(hostname -f)/emailAddress=admin@localhost" \
...
setup_cronjobs() {
    # Reboot automatique SUPPRIMÉ (trop dangereux)
...
update_banner() {
    wget -q -O /etc/issue.net "${SERVER_HOST}/module/issue.net" || true
...
install_ws_scripts() {
    ...
    wget -q -O /usr/local/bin/ws-dropbear "${SERVER_HOST}/module/dropbear-ws.py" || {
        echo "[ERROR] Impossible de télécharger dropbear-ws.py"
        return 1
    }
    wget -q -O /usr/local/bin/ws-stunnel "${SERVER_HOST}/module/ws-stunnel" || {
        echo "[ERROR] Impossible de télécharger ws-stunnel"
        return 1
    }
...
install_ws_services() {
    ...
    wget -q -O /etc/systemd/system/ws-dropbear.service "${SERVER_HOST}/module/service-wsdropbear" || {
        echo "[ERROR] Impossible de télécharger service-wsdropbear"
        return 1
    }
    wget -q -O /etc/systemd/system/ws-stunnel.service "${SERVER_HOST}/module/ws-stunnel.service" || {
        echo "[ERROR] Impossible de télécharger ws-stunnel.service"
        return 1
    }
